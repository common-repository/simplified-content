...
class OTLParser {
    ...
    public function parse($text, $params) {
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'OTLPARSE');
    ...
    private function substituteBracedFields($matches) {
        ...
        if ($propertyFound == true) {
            ...
            for ($i = 0; $i < sizeof($chain); $i = $i + 2) {
                ...
                if ($delimiter == ".") {
                    $string = "OTLParser::getObjectProperty(" . $string . ",'" . $memberName . "')";
                } else if ($delimiter == ":") {
            ...
            if ($this->paramsArray) {
                ...
                for ($i = 1; $param && $i < $chainLength; $i++) {
                    if (is_array($param)) {
                        ...
                        $param = OTLParser::getObjectProperty($param, $chain [$i]);
                    }
                }
    ...
    // Get property via a getter if it exists
    public static function getObjectProperty($object, $property) {
        $targetMethod = "get" . strtoupper(substr($property, 0, 1)) . substr($property, 1);
